...
function loadSettingsForm() {
  ...
  val('set-notif-phone',   S.biz.notifPhone || '');
...
async function saveBranding() {
  ...
  var rawPhone     = gval('set-notif-phone').trim();
  S.biz.notifPhone = rawPhone ? normalizePhone(rawPhone) : '';
  await saveBizFS();
...
function showInfo(btn, text) {
  ...
  /* Baca teks dari data-info attribute */
  var msg = text || (btn && btn.getAttribute ? btn.getAttribute('data-info') : '') || '';
  if (!msg) return;

  /* Isi konten - textContent aman dari XSS */
  cnt.textContent = msg;

  /* Hitung posisi relatif viewport (bukan dokumen) */
  var rect = btn.getBoundingClientRect();
  var vw   = window.innerWidth;
  var vh   = window.innerHeight;
  var pw   = Math.min(290, vw - 24);
  var left = rect.left;
  /* Jangan keluar kanan */
  if (left + pw > vw - 8) left = vw - pw - 8;
  if (left < 8) left = 8;
  /* Tampilkan di bawah tombol, atau di atas jika tidak cukup ruang */
  var topBelow = rect.bottom + 8;
  var topAbove = rect.top - 8;

  /* Set style langsung - position:fixed agar tidak terpengaruh scroll */
  pop.style.cssText = ''
    + 'display:block;'
    + 'position:fixed;'
    + 'left:' + Math.round(left) + 'px;'
    + 'top:'  + Math.round(topBelow) + 'px;'
    + 'max-width:' + pw + 'px;'
    + 'z-index:99999;'
    + 'opacity:0;'
  ...
  /* Fade in di frame berikutnya */
  requestAnimationFrame(function() {
    pop.style.opacity = '1';
    pop.style.transform = 'translateY(0) scale(1)';
  });

  /* Tutup saat klik di luar - hapus listener lama dulu */
  document.removeEventListener('click', _closeInfoOnOutside, true);
  setTimeout(function() {
...
function _renderCalendarSync() {
  ...
  for(var d=1;d<=days;d++){
    ...
    if(!isClosed&&!isPast){
      if(avail==='full')    extraCls+=' cal-day-full';
      else if(avail==='busy') extraCls+=' cal-day-busy';
    }

    /* Angka reservasi dengan warna gradual - hanya jika ada reservasi */
    var pillHtml='';
    if(cnt){
      var pillIcon=avail==='full'?'&#x25CF;':avail==='busy'?'&#x25D4;':'&#x2713;';
      pillHtml='<div class="cal-res-pill">'
        +'<span style="font-size:.58rem;opacity:.7">'+pillIcon+'</span>'
        +cnt+' res'
        +'</div>';
    }
...
function renderMenusTable() {
  ...
  var menus=getMenusSorted();
  if(!menus.length){tbody.innerHTML='<tr><td colspan="5" style="text-align:center;padding:36px;color:var(--ink-4)"><div style="font-size:2rem;margin-bottom:10px">???????</div><div style="font-weight:600">Belum ada menu</div></td></tr>';return;}
...
var NOTIF = {
  ...
  render: function() {
    var newRes   = this.getNewRes();
    var thankRes = this.getPending();
    var total    = newRes.length + thankRes.length;
    ...
    /* Section reservasi baru */
    var newSection = document.getElementById('notif-new-section');
    var newList    = document.getElementById('notif-new-list');
    if (newSection && newList) {
      if (newRes.length) {
        newSection.style.display = 'block';
        newList.innerHTML = newRes.map(function(r) {
          var tp = Array.isArray(r.tempat) ? r.tempat.join(', ') : (r.tempat || '-');
          var info = formatDateDisp(r.date) + '  -  ' + (r.jam||'-') + '  -  ' + r.jumlah + ' orang  -  ' + tp;
          return '<div class="notif-new-item">'
            + '<div class="nni-name">' + esc(r.nama) + '</div>'
            + '<div class="nni-info">' + esc(info) + (r.tambahan ? '<br>\ud83d\udcdd ' + esc(r.tambahan) : '') + '</div>'
            + (isReadOnly() ? '' :
                '<div class="nni-actions">'
                + '<button class="nni-btn nni-btn-ok" onclick="notifConfirm(\'' + r.id + '\',event)"><i class="fas fa-check"></i> Konfirmasi</button>'
                + '<button class="nni-btn nni-btn-no" onclick="notifReject(\'' + r.id + '\',event)"><i class="fas fa-times"></i> Tolak</button>'
                + '</div>')
            + '</div>';
        }).join('');
      } else {
    ...
    var thanksTitle = document.getElementById('nd-thanks-title');
    if (!listEl) return;
    if (thanksTitle) thanksTitle.style.display = thankRes.length ? 'flex' : 'none';
    if (!thankRes.length) {
      listEl.innerHTML = newRes.length ? '' : '<div class="nd-empty"><i class="fas fa-check-circle" style="color:var(--success);font-size:1.3rem;display:block;margin-bottom:6px"></i>Semua beres!</div>';
      return;
    }
    listEl.innerHTML = thankRes.map(function(r){
  ...
  showToastIfNew: function() {
    if (this._toastShown) return;
    this._toastShown = true;
    var n = this.getNewRes().length;
    if (!n) return;
    var msg = n === 1
      ? '\ud83d\udd14 Ada 1 reservasi baru yang perlu dikonfirmasi!'
      : '\ud83d\udd14 Ada ' + n + ' reservasi baru yang perlu dikonfirmasi!';
...
window.notifConfirm = function(id, e) {
  if (e) e.stopPropagation();
  quickStatus(id, 'confirmed');
  setTimeout(function(){ NOTIF.render(); }, 500);
};
window.notifReject = function(id, e) {
  if (e) e.stopPropagation();
  quickStatus(id, 'batal');
  setTimeout(function(){ NOTIF.render(); }, 500);
};
