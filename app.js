...
/* [#1] Tampilkan badge reservasi hari ini di nav */
updateNavBadge();
...
initApp();
showToast(‘Selamat datang di Proserva! Selamat bekerja!’, ‘success’, 4000);
}
...
var top = Object.entries(dayCounts).sort(function(a, b) {
return b[1] - a[1];
})[0];
...
/* [#1] Update badge setelah render kalender */
updateNavBadge();
}
...
/* [#3] Reset filter status bar ke “Semua” saat pindah tanggal */
resetStatusFilter();
...
thankBtn =
’<button class=“btn-secondary” style=“font-size:0.78rem;padding:7px 11px;” ’ +
...
/* [#5] Inline status dropdown di footer kartu */
var statusSelect = buildStatusSelect(r);
...
/* [#7] Reset status picker ke “pending” */
resetStatusPicker();
...
document.getElementById(‘res-date’).value     = r.date      || todayStr();
...
/* [#7] Set status picker sesuai data yang diedit */
setStatusPicker(r.status || ‘pending’);
...
/* [#7] Baca status dari hidden input (diisi oleh pickStatus) */
var status = (document.getElementById(‘res-status’).value) || ‘pending’;
...
if (!dateVal) {
showFieldError(‘err-date’, ‘Tanggal wajib diisi’);
valid = false;
}
...
usedNames[val] = true;
menus.push({ name: val, quantity: qty });
```
...
/* Gunakan tanggal dari form (bukan selectedDate saja) */
var dateStr = dateVal || state.selectedDate || todayStr();
...
/* Refresh list jika tanggal yang diedit = tanggal yang sedang ditampilkan */
...
/* [#4] Hapus reservasi — dialog konfirmasi dinamis */
function deleteRes(id) {
var r    = findReservationById(id);
var name = r ? r.nama : ‘reservasi ini’;
var jam  = r ? r.jam  : ‘’;

var msg = ’Hapus reservasi atas nama ’ + name +
(jam ? ’, jam ’ + jam : ‘’) + ‘?’;

showConfirmModal(msg, ‘Konfirmasi Hapus’, function() {
...
/* ============================================================
8. MENU ROWS (multi-item picker di reservation modal)
============================================================ */
...
escapeHtml(m.name) + price + ‘</option>’;
}).join(’’);
...
if (hint) {
hint.textContent = loc ? ‘Kapasitas: ’ + loc.capacity + ’ orang’ : ‘’;
}
}
...
if (menus.length === 0) {
tbody.innerHTML =
‘<tr><td colspan="4" style="text-align:center;padding:24px;color:var(--ink-4);">’ +
...
showConfirmModal(
‘Hapus menu “’ + name + ‘”? Menu ini juga akan hilang dari semua reservasi yang sudah ada.’,
‘Hapus Menu’,
function() {
...
return (
  '<tr>' +
    ...
    '<td>' + (c.lastDate ? formatDateDisplay(c.lastDate) : '&mdash;') + '</td>' +
    '<td>' + waBtn + '</td>' +
  '</tr>'
);
```
...
custCounts[r.nomorHp] = { name: r.nama, count: 0 };
}
...
return ‘<li class="rank-item"><span class="ri-name">’ + escapeHtml(c.name) +
‘</span><span class="ri-val">’ + c.count + ‘x</span></li>’;
}).join(’’)
...
return ‘<li class="rank-item"><span class="ri-name">’ + escapeHtml(m.key) +
‘</span><span class="ri-val">’ + m.count + ’ porsi</span></li>’;
}).join(’’)
: ‘<li style="color:var(--ink-4);font-size:0.85rem;padding:12px 0;">Belum ada data</li>’;
}
...
if (sel) sel.className = ‘rc-status-select rc-status-’ + newStatus;

showToast(’Status diubah: ’ + (STATUS_LABELS[newStatus] || newStatus), ‘success’);
updateNavBadge();
}

/* ============================================================
[#7] STATUS PICKER di form reservasi
============================================================ */
...
if (el) {
el.innerHTML =
‘<span class="tb-page-name">’ + (map[name] || name) + ‘</span>’;
}

document.title = (map[name] || name) + ’ - Proserva’;
}
...
return parseInt(parts[2], 10) + ’ ’ + (MONTHS_ID[parseInt(parts[1], 10) - 1] || ‘’) + ’ ’ + parts[0];
}
...
var mk  = getMonthKey(parseInt(parts[0], 10), parseInt(parts[1], 10) - 1);
var arr = state.reservations[mk] || [];
return arr.filter(function(r) { return r.date === dateStr; });
}
...
if (!map[key]) {
map[key] = { nama: r.nama || ‘Tanpa Nama’, nomorHp: r.nomorHp || ‘’, count: 0, lastDate: r.date };
}
map[key].count++;
...
/* ============================================================
21. FINALIZATION & DEBUG
============================================================ */
(function() {
try {
if (!window.state)      console.warn(’[Proserva] state belum terinisialisasi’);
if (!window.DB)         console.warn(’[Proserva] DB belum tersedia’);
if (!window.KEYS)       console.warn(’[Proserva] KEYS belum tersedia’);
if (!window.getMonthKey) console.warn(’[Proserva] getMonthKey tidak ditemukan’);
} catch (e) {
...
var header   = document.getElementById(‘topbar’) || document.querySelector(’.topbar’);
if (!header) return;
