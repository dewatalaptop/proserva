...
/* ──────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────── */
var _UID      = '';
var _ACCOUNT  = null;   /* data akun dari Firestore: { status, trialEndsAt, plan, ... } */
...
/* ──────────────────────────────────────────────────────────
   READ-ONLY GUARD
   Blokir semua operasi write jika mode read-only
   ────────────────────────────────────────────────────────── */
function isReadOnly() {
  return !!window._READ_ONLY;
}
...
/**
 * Baca dokumen akun dari Firestore.
 * Jika belum ada (user lama sebelum sistem trial),
 * buat dokumen dengan trial 14 hari dari sekarang.
 */
...
function evaluateAccountStatus() {
  ...
  if (status === 'trial') {
    ...
    if (now > end) {
      /* Update status ke expired di Firestore */
...
function trialDaysLeft() {
  ...
  var ms   = _ACCOUNT.trialEndsAt - Date.now();
  var days = Math.ceil(ms / (24 * 60 * 60 * 1000));
  return Math.max(0, days);
}

/**
 * Update status akun di Firestore (fire-and-forget)
 */
async function _updateAccountStatus(newStatus, extra) {
  ...
  try {
    ...
    await fb.setDoc(
      fb.doc(fb.db, 'users', _UID, 'config', 'account'),
      update,
      { merge: true }
    );
    if (_ACCOUNT) Object.assign(_ACCOUNT, update);
  } catch (e) {
    console.warn('_updateAccountStatus error:', e);
  }
}
...
function applyAccountUI(accountStatus) {
  ...
  if (accountStatus === 'active') {
    ...
    if (sbStatus) {
      ...
      sbStatus.innerHTML = '<i class="fas fa-clock"></i> Trial · ' + days + ' hari tersisa';
    }
    if (trialBanner) {
      trialBanner.style.display = 'flex';
      var txt = document.getElementById('trial-banner-text');
      if (txt) {
        txt.textContent = days <= 1
  ...
  /* Render info di halaman pengaturan */
...
function renderAccountStatusCard(status) {
  ...
  var s = statusMap[status] || statusMap.trial;

  var daysLeftHtml = '';
  if (status === 'trial') {
    var d = trialDaysLeft();
    daysLeftHtml = '<div style="margin-top:6px;font-size:.78rem;color:var(--ink-4)">Berakhir: <strong style="color:var(--ink-2)">'
      + formatDateDisp(new Date(_ACCOUNT.trialEndsAt).toISOString().split('T')[0])
      + '</strong> · ' + d + ' hari lagi</div>';
  }
  if (status === 'active') {
    daysLeftHtml = '<div style="margin-top:6px;font-size:.78rem;color:var(--ink-4)">Paket: <strong style="color:var(--ink-2)">'
  ...
  el.innerHTML = '<div style="display:flex;align-items:center;gap:9px">'
    ...
    + daysLeftHtml + '</div></div>';

  /* Tampilkan atau sembunyikan billing section */
...
/* ──────────────────────────────────────────────────────────
   ...
   Dipanggil dari index.html setelah Midtrans snap.pay onSuccess
   ────────────────────────────────────────────────────────── */
window.onPaymentSuccess = async function (planType, midtransResult) {
  ...
  var msMonth = 31  * 24 * 60 * 60 * 1000;
  var activeUntil = now + (planType === 'yearly' ? msYear : msMonth);
  ...
  if (_ACCOUNT) {
    ...
    _ACCOUNT.plan        = planType;
    _ACCOUNT.activeUntil = activeUntil;
  }
...
async function loadStateFS() {
  ...
  try {
    /* Biz */
    var bizSnap = await fb.getDoc(fb.doc(fb.db, 'users', _UID, 'config', 'biz'));
    ...
    var opsSnap = await fb.getDoc(fb.doc(fb.db, 'users', _UID, 'config', 'ops'));
    S.ops = opsSnap.exists()
      ? Object.assign(opsDef, opsSnap.data())
      : Object.assign(opsDef, DB.get(K.OPS, {}));

    /* Messages */
    var msgsSnap = await fb.getDoc(fb.doc(fb.db, 'users', _UID, 'config', 'msgs'));
...
window._onAuthReady = async function (user) {
  ...
  /* Tunggu Firebase module siap */
  var waitCount = 0;
  while (!window._DONE_FS && waitCount < 40) {
  ...
  if (!Object.keys(S.locs || {}).length) {
    ...
    showScreen('app');
    /* Apply UI trial/active setelah app terlihat */
...
/* ──────────────────────────────────────────────────────────
   ACCENT (satu-satunya appearance setting yang tersisa)
   ────────────────────────────────────────────────────────── */
var ACCENT_MAP = {
  blue:    '#2563eb', teal:   '#0d9488', violet: '#7c3aed',
  orange:  '#ea580c', rose:   '#e11d48', emerald:'#059669'
};
...
function buildEmojiGrid() {
  ...
  g.innerHTML = LOGO_EMOJIS.map(function (em) {
    var active = em === cur ? ' active' : '';
    return '<div class="emoji-opt' + active + '" data-emoji="' + em + '" onclick="pickEmoji(\'' + em + '\')">' + em + '</div>';
  }).join('');
}
...
function showView(name) {
  ...
  if (viewActions[name]) viewActions[name]();
}
...
function checkConflict(date, locName, jam, jumlah, durationOverride, excludeId) {
  ...
  if (pax > capacity) return { ok:false, type:'hard_capacity', msg:'✗ Melebihi kapasitas (maks. ' + capacity + ' orang)' };
...
async function wzFinish() {
  ...
  try {
    ...
    showScreen('app');
    var accountStatus = evaluateAccountStatus();
    requestAnimationFrame(function () { applyAccountUI(accountStatus); });
...
function buildResCard(r) {
  ...
  /* Footer: read-only hanya tampilkan status tanpa aksi write */
  var footerHtml = '';
  if (isReadOnly()) {
...
function renderMenusTable() {
  ...
  var menus=getMenusSorted();
  if (!menus.length) {
    tbody.innerHTML='<tr><td colspan="4" style="text-align:center;padding:36px;color:var(--ink-4)"><div style="font-size:2rem;margin-bottom:10px">🍽️</div><div style="font-weight:600">Belum ada menu</div></td></tr>';
    return;
  }
  tbody.innerHTML = menus.map(function (m) {
    var editBtn = isReadOnly() ? '' : '<button class="btn btn-info btn-sm" onclick="openMenuModal(\''+m.id+'\')"><i class="fas fa-edit"></i></button><button class="btn btn-danger btn-sm" onclick="doDeleteMenu(\''+m.id+'\')"><i class="fas fa-trash-alt"></i></button>';
...
function renderLocsTable() {
  ...
  var locs=getLocsSorted();
  if (!locs.length) {
    tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:36px;color:var(--ink-4)"><div style="font-size:2rem;margin-bottom:10px">📍</div><div style="font-weight:600">Belum ada lokasi</div></td></tr>';
    return;
  }
...
function doPrint() {
  ...
  var items=res.map(function(r,i){
    ...
    if (opts.menu&&Array.isArray(r.menus)&&r.menus.length) {
      menuH='<p style="font-weight:600;margin-top:8px">🍽 Pesanan:</p><ul>'+r.menus.map(function(m){var md=getMenuByName(m.name),det=(md&&md.details)||[];return '<li><strong>'+m.quantity+'x '+esc(m.name)+'</strong>'+(det.length?'<br><small style="color:#555">'+det.map(esc).join(', ')+'</small>':'')+' </li>';}).join('')+'</ul>';
    }
...
function handleExport() {
  var payload={ v:4, exportedAt:new Date().toISOString(), biz:S.biz, menus:S.menus, locs:S.locs, res:S.res, ops:S.ops };
  var code='';
...
async function doImport() {
  ...
  DB.set(K.BIZ,S.biz); DB.set(K.MENUS,S.menus); DB.set(K.LOCS,S.locs); DB.set(K.RES,S.res); DB.set(K.OPS,S.ops);
  if (_UID&&window._FB) {
    var fb=window._FB;
    try {
      await fb.setDoc(fb.doc(fb.db,'users',_UID,'config','biz'),S.biz);
      ...
      _showSync();
    } catch(e){ console.warn('Import FS sync:',e); }
  }
...
function runAnalysis() {
  ...
  var dowMap={}; DAYS.forEach(function(d){dowMap[d]=0;}); filtered.forEach(function(r){if(r.date){var dow=DAYS[new Date(r.date+'T12:00:00').getDay()];dowMap[dow]=(dowMap[dow]||0)+1;}});
  ...
  if (insightEl) insightEl.innerHTML='<h5><i class="fas fa-lightbulb"></i> Insight Otomatis</h5>'+(filtered.length?'<ul>'+ins.map(function(i){return '<li>'+i+'</li>';}).join('')+'</ul>':'<p style="color:var(--ink-4);font-size:.85rem">Belum ada data untuk periode ini.</p>');
...
function renderAnlChart(labels,data,title) {
  ...
  var acColor=getComputedStyle(document.documentElement).getPropertyValue('--ac').trim()||'#2563eb';
  anlChart=new Chart(ctx.getContext('2d'),{
    type:'bar',
    data:{labels,datasets:[{label:'Reservasi',data,backgroundColor:function(c){var ch=c.chart,ct=ch.ctx,a=ch.chartArea;if(!a)return acColor+'b3';var g=ct.createLinearGradient(0,a.top,0,a.bottom);g.addColorStop(0,acColor+'cc');g.addColorStop(1,acColor+'22');return g;},borderRadius:7,borderSkipped:false,hoverBackgroundColor:acColor}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:{display:true,text:title,font:{size:13,weight:'600'},color:'rgba(15,23,42,.4)',padding:{bottom:14}},tooltip:{backgroundColor:'#fff',titleColor:'#0f172a',bodyColor:'#475569',borderColor:'rgba(15,23,42,.1)',borderWidth:1,padding:10,cornerRadius:8,callbacks:{label:function(c){return ' '+c.raw+' reservasi';}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10},color:'rgba(15,23,42,.4)'}},y:{beginAtZero:true,ticks:{stepSize:1,font:{size:10},color:'rgba(15,23,42,.4)'},grid:{color:'rgba(15,23,42,.05)'}}}}
  });
}
