‘use strict’;
/* ═══════════════════════════════════════════════════
PROSERVA v4.0 — app.js
═══════════════════════════════════════════════════ */

/* ── Storage Keys ── */
var K = {
BIZ:‘psv_biz’, MENUS:‘psv_menus’, LOCS:‘psv_locs’, RES:‘psv_res’,
BC_MSG:‘psv_bc_msg’, APPEAR:‘psv_appear’, OPS:‘psv_ops’, MSGS:‘psv_msgs’
};
var _UID = ‘’;
...
var DB = {
get: function(k, fb) {
try { var r = localStorage.getItem(_ckey(k)); return r !== null ? JSON.parse(r) : (fb !== undefined ? fb : null); }
...
/* ═══════════════════════════════════════════════════
FIREBASE HELPERS (Dual Storage)
═══════════════════════════════════════════════════ */
async function saveBizFS() {
DB.set(K.BIZ, S.biz); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘config’,‘biz’), S.biz); } catch(e) { console.warn(e); }
}
async function saveMenuFS(id, data) {
S.menus[id] = data; DB.set(K.MENUS, S.menus); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘menus’,id), data); } catch(e) { console.warn(e); }
}
async function deleteMenuFS(id) {
delete S.menus[id]; DB.set(K.MENUS, S.menus); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.deleteDoc(fb.doc(fb.db,‘users’,_UID,‘menus’,id)); } catch(e) { console.warn(e); }
}
async function saveLocFS(id, data) {
S.locs[id] = data; DB.set(K.LOCS, S.locs); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘locations’,id), data); } catch(e) { console.warn(e); }
}
async function deleteLocFS(id) {
delete S.locs[id]; DB.set(K.LOCS, S.locs); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.deleteDoc(fb.doc(fb.db,‘users’,_UID,‘locations’,id)); } catch(e) { console.warn(e); }
}
...
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘reservations’,mk,‘list’,r.id), r); } catch(e) { console.warn(e); }
}
...
DB.set(K.RES, S.res); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.deleteDoc(fb.doc(fb.db,‘users’,_UID,‘reservations’,mk,‘list’,r.id)); } catch(e) { console.warn(e); }
}
async function saveOpsFS() {
DB.set(K.OPS, S.ops); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘config’,‘ops’), S.ops); } catch(e) { console.warn(e); }
}
async function saveMsgsFS() {
DB.set(K.MSGS, S.msgs); _showSync();
if (!_UID || !window._FB) return;
try { var fb = window._FB; await fb.setDoc(fb.doc(fb.db,‘users’,_UID,‘config’,‘msgs’), S.msgs); } catch(e) { console.warn(e); }
}
...
S.ops = opsS.exists() ? Object.assign({openTime:‘09:00’,closeTime:‘21:00’,slotInterval:30,defaultDuration:120,bufferTime:15,minAdvance:2}, opsS.data()) : Object.assign({openTime:‘09:00’,closeTime:‘21:00’,slotInterval:30,defaultDuration:120,bufferTime:15,minAdvance:2}, DB.get(K.OPS, {}));
var msgsS = await fb.getDoc(fb.doc(fb.db,‘users’,_UID,‘config’,‘msgs’));
S.msgs = msgsS.exists() ? Object.assign({}, S.msgs, msgsS.data()) : Object.assign({ confirm: DEFAULT_CONF, thanks: DEFAULT_THANKS }, DB.get(K.MSGS, {}));
...
} catch(e) { console.warn(‘loadStateFS fallback:’, e); loadState(); }
...
if (!window._FB) { showToast(‘Firebase belum dikonfigurasi!’, ‘error’); return; }
...
var msg = e.code === ‘auth/popup-closed-by-user’ ? ‘Login dibatalkan.’ : ’Gagal login: ’ + e.message;
showToast(msg, ‘error’);
if (btn) { btn.disabled = false; btn.innerHTML = ‘<svg viewBox="0 0 24 24" width="20" height="20"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg> Lanjutkan dengan Google’; }
}
}

async function doSignOut() {
if (!confirm(‘Yakin ingin keluar dari Proserva?’)) return;
if (window._FB) { try { await window._FB.signOut(window._FB.auth); } catch(e) { console.warn(e); } }
_UID = ‘’; S.res = {}; S.menus = {}; S.locs = {};
showScreen(‘landing’);
showToast(‘Berhasil keluar. Sampai jumpa! 👋’, ‘info’);
}

/* ═══════════════════════════════════════════════════
ON AUTH READY (dipanggil dari Firebase module)
═══════════════════════════════════════════════════ */
...
var avatarFb = document.getElementById(‘sb-avatar-fallback’);
...
avatarFb.textContent = initials(user.displayName || ‘?’);
}
}
...
═══════════════════════════════════════════════════ */
function boot() {
buildAccentGrid(); buildEmojiGrid(); initModalClose(); initKbd();
document.getElementById(‘sidebar-overlay’).onclick = toggleSidebar;
// Fallback timeout 8 detik jika auth tidak kunjung selesai
setTimeout(function() {
var el = document.getElementById(‘screen-loading’);
if (el && el.classList.contains(‘active’)) showScreen(‘landing’);
}, 8000);
}
...
menus:  {}, locs: {}, res: {},
month:  new Date().getMonth(), year: new Date().getFullYear(), date: null,
anlChart: null, bcList: [],
...
var duration = getEffectiveDuration(loc), buffer = getEffectiveBuffer(loc), interval = S.ops.slotInterval || 30;
var openM = toMins(loc.openTime || S.ops.openTime || ‘09:00’), closeM = toMins(loc.closeTime || S.ops.closeTime || ‘21:00’);
...
VIEW ROUTER
═══════════════════════════════════════════════════ */
var PAGE_NAMES = { calendar:‘Kalender’, detail:‘Detail Reservasi’, menus:‘Menu & Paket’, locations:‘Lokasi’, customers:‘Pelanggan’, analysis:‘Analisis Bisnis’, broadcast:‘Broadcast Promo’, settings:‘Pengaturan’ };
...
var actions = { menus:renderMenusTable, locations:renderLocsTable, customers:function(){renderCustomers(’’);}, analysis:function(){setupAnlSelectors();runAnalysis();}, broadcast:loadBcView, settings:loadSettingsForm };
...
val(‘set-biz-name’, S.biz.name || ‘’); val(‘set-tagline’, S.biz.tagline || ‘’); selVal(‘set-biz-type’, S.biz.type || ‘restoran’);
val(‘set-open’, S.ops.openTime || ‘09:00’); val(‘set-close’, S.ops.closeTime || ‘21:00’);
selVal(‘set-slot-interval’, String(S.ops.slotInterval || 30));
val(‘set-duration’, S.ops.defaultDuration || 120); val(‘set-buffer’, S.ops.bufferTime || 15); val(‘set-min-advance’, S.ops.minAdvance || 2);
val(‘set-wa-confirm’, S.msgs.confirm || ‘’); val(‘set-wa-thanks’, S.msgs.thanks || ‘’);
applyAllAppearance(false);
}
async function saveBranding() {
S.biz.name = gval(‘set-biz-name’).trim() || ‘Usaha Saya’; S.biz.type = gval(‘set-biz-type’);
S.biz.tagline = gval(‘set-tagline’).trim(); S.biz.logo = S.appear.logo || ‘🍽️’;
await saveBizFS();
setText(‘sb-biz-name’, S.biz.name); setText(‘cal-title’, ’Dashboard - ’ + S.biz.name);
showToast(‘Branding berhasil disimpan!’, ‘success’);
}
async function saveOperational() {
S.ops = { openTime:gval(‘set-open’)||‘09:00’, closeTime:gval(‘set-close’)||‘21:00’, slotInterval:parseInt(gval(‘set-slot-interval’))||30, defaultDuration:parseInt(gval(‘set-duration’))||120, bufferTime:parseInt(gval(‘set-buffer’))||15, minAdvance:parseInt(gval(‘set-min-advance’))||2 };
await saveOpsFS(); showToast(‘Pengaturan operasional disimpan!’, ‘success’);
}
async function saveMessages() {
S.msgs.confirm = gval(‘set-wa-confirm’); S.msgs.thanks = gval(‘set-wa-thanks’);
await saveMsgsFS(); showToast(‘Template pesan disimpan!’, ‘success’);
}
...
setText(‘wz-err-1’, ‘’); S.wizData.bizName = n; S.wizData.bizType = gval(‘wz-biz-type’);
}
if (step === 3) {
...
var el = document.getElementById(‘wz-loc-list’); if (!el) return;
...
var el = document.getElementById(‘wz-menu-list’); if (!el) return;
...
S.wizData.locs.push({ name:n, capacity:c }); val(‘wz-loc-name’, ‘’); val(‘wz-loc-cap’, ‘’); renderWzLocList();
}
function wzAddMenu() {
...
val(‘wz-menu-name’, ‘’); val(‘wz-menu-price’, ‘’); val(‘wz-menu-detail’, ‘’); renderWzMenuList();
}
...
var biz = S.biz.name;
setText(‘cal-title’, ’Dashboard - ’ + biz); setText(‘cal-sub’, ‘Selamat datang kembali!’); setText(‘sb-biz-name’, biz);
applyAllAppearance(false); renderCalendar(); NOTIF.start();
document.removeEventListener(‘click’, closeNotifH); document.addEventListener(‘click’, closeNotifH);
loadSettingsForm(); showScreen(‘app’); showToast(‘Selamat datang di Proserva! 🎉’, ‘success’, 4000);
} catch(e) {
...
setText(‘st-total’, monthRes.length); setText(‘st-pax’, totalPax);
setText(‘st-dp’, ‘Rp’ + formatRpK(totalDp)); setText(‘st-busy’, busiestDay);
var today = new Date(), calEl = document.getElementById(‘cal-days’); if (!calEl) return;
var html = ‘’;
...
var addBtn = document.getElementById(‘btn-add-res’); if (addBtn) addBtn.style.display = ‘none’;
setText(‘topbar-page’, ‘Detail Reservasi’);
renderAvailBar(ds); renderDetail(getResDate(ds));
...
return (r.nama && r.nama.toLowerCase().includes(ql)) || (r.tempat && r.tempat.toLowerCase().includes(ql))
|| (r.nomorHp && r.nomorHp.includes(q)) || (Array.isArray(r.menus) && r.menus.some(function(m) { return m.name.toLowerCase().includes(ql); }));
}));
}
...
+ ‘<button class="btn btn-danger btn-sm" onclick="delRes(\'' + r.id + '\')"><i class="fas fa-trash-alt"></i></button></div></div>’;
}
...
val(‘res-date’, S.date || todayStr()); val(‘res-jam’, ‘’); val(‘res-jumlah’, ‘’);
val(‘res-dp’, ‘0’); val(‘res-duration’, ‘’); selVal(‘res-tipe-dp’, ‘’); selVal(‘res-status’, ‘pending’);
setText(‘res-cap-hint’, ‘’); setText(‘res-dur-hint’, ‘’);
populateLocSelect(‘res-tempat’, ‘’); clearConflictUI();
var mc = document.getElementById(‘res-menus-container’); if (mc) mc.innerHTML = ‘’;
addMenuRow(‘res-menus-container’); openModal(‘modal-res’);
}
function openEditRes(id) {
var r = findRes(id); if (!r) { showToast(‘Tidak ditemukan!’, ‘error’); return; }
clearErrors(); clearConflictUI(); val(‘res-edit-id’, id);
setHTML(‘res-modal-title’, ‘<i class="fas fa-edit"></i> Edit Reservasi’);
val(‘res-nama’, r.nama||’’); val(‘res-hp’, r.nomorHp||’’); val(‘res-date’, r.date||todayStr());
val(‘res-jam’, r.jam||’’); val(‘res-jumlah’, r.jumlah||’’); val(‘res-dp’, r.dp||0);
val(‘res-duration’, r.duration||’’); val(‘res-tambahan’, r.tambahan||’’);
selVal(‘res-tipe-dp’, r.tipeDp||’’); selVal(‘res-status’, r.status||‘pending’);
populateLocSelect(‘res-tempat’, r.tempat); onResFieldChange();
...
var ca = document.getElementById(‘conflict-alert’); if (ca) { ca.className=‘conflict-alert’; ca.innerHTML=’’; }
var ss = document.getElementById(‘slot-suggester’); if (ss) ss.classList.remove(‘show’);
var sb = document.getElementById(‘btn-save-res’); if (sb) sb.disabled = false;
}
...
if (!ca) return;
if (result.type === ‘none’)          { ca.className=‘conflict-alert conflict-ok show’;   ca.innerHTML=’<i class="fas fa-check-circle"></i> ‘+result.msg; ss.classList.remove(‘show’); if(sb)sb.disabled=false; return; }
if (result.type === ‘soft_capacity’) { ca.className=‘conflict-alert conflict-soft show’; ca.innerHTML=’<i class="fas fa-exclamation-triangle"></i> ‘+result.msg; ss.classList.remove(‘show’); if(sb)sb.disabled=false; return; }
ca.className=‘conflict-alert conflict-hard show’; ca.innerHTML=’<i class="fas fa-times-circle"></i> ‘+result.msg; if(sb)sb.disabled=true;
...
if (html) { document.getElementById(‘slot-content’).innerHTML = html; ss.classList.add(‘show’); } else ss.classList.remove(‘show’);
}
...
usedN[sel.value] = true; menus.push({ name:sel.value, quantity:qty });
});
...
var resObj;
if (editId) {
var ex = findRes(editId); if (!ex) { showToast(‘Tidak ditemukan!’, ‘error’); return; }
...
if (S.date) renderDetail(getResDate(S.date)); renderCalendar();
}
...
if (S.date) { renderDetail(getResDate(S.date)); renderAvailBar(S.date); } renderCalendar();
}
...
opt.value = loc.name; opt.textContent = loc.name + ’ (maks. ’ + loc.capacity + ’ org)’;
...
if (!menus.length) { c.innerHTML = ‘<div style="padding:12px;text-align:center;color:var(--ink-4);font-size:.82rem">Belum ada menu. <span onclick="showView(\'menus\')" style="color:var(--ac);cursor:pointer">Tambah dulu</span></div>’; return; }
var opts = menus.map(function(m) { return ‘<option value=”’ + esc(m.name) + ‘”’ + (m.name===menuName?’ selected’:’’) + ‘>’ + esc(m.name) + (m.price ? ’ — Rp’ + formatRp(m.price) : ‘’) + ‘</option>’; }).join(’’);
var div = document.createElement(‘div’); div.className = ‘menu-row’;
...
editId = editId || null; val(‘menu-edit-id’, editId || ‘’);
...
renderMenusTable(); closeModal(‘modal-menu’); showToast(‘Menu “’ + name + ‘” disimpan!’, ‘success’);
}
async function doDeleteMenu(id) {
var n = S.menus[id] ? S.menus[id].name : ‘menu ini’;
if (!confirm(‘Hapus menu “’ + n + ‘”?’)) return;
await deleteMenuFS(id); renderMenusTable(); showToast(‘Menu dihapus’, ‘info’);
}
...
editId = editId || null; val(‘loc-edit-id’, editId || ‘’);
...
var data = { name, capacity:cap, minGuests:parseInt(gval(‘loc-min’))||1, defaultDuration:gval(‘loc-duration’)?parseInt(gval(‘loc-duration’)):’’, bufferTime:gval(‘loc-buffer’)!==’’?parseInt(gval(‘loc-buffer’)):’’, openTime:gval(‘loc-open’), closeTime:gval(‘loc-close’) };
await saveLocFS(editId || genId(), data);
renderLocsTable(); closeModal(‘modal-loc’); showToast(‘Lokasi “’ + name + ‘” disimpan!’, ‘success’);
}
async function doDeleteLoc(id) {
if (!confirm(‘Hapus lokasi “’ + (S.locs[id]?S.locs[id].name:‘ini’) + ‘”?’)) return;
await deleteLocFS(id); renderLocsTable(); showToast(‘Lokasi dihapus’, ‘info’);
}
...
var k = r.nomorHp || (’_*np*’ + (r.nama||’’).toLowerCase().trim());
...
WHATSAPP
═══════════════════════════════════════════════════ */
function openWA(phone, msg) { if (!phone) return; var f = phone.replace(/^0/, ‘62’); window.open(‘https://wa.me/’ + f + ‘?text=’ + encodeURIComponent(msg), ‘_blank’, ‘noopener’); }
function contactWA(id) { var r = findRes(id); if (!r||!r.nomorHp) { showToast(‘Nomor HP tidak ada’,‘error’); return; } openWA(r.nomorHp, buildConfMsg(r)); }
async function sendThankYou(id) {
var r = findRes(id); if (!r||!r.nomorHp) { showToast(‘Nomor HP tidak ada’,‘error’); return; }
...
if (card) { var btn = card.querySelector(’[onclick*=“sendThankYou”]’); if (btn) btn.outerHTML = ‘<button class="btn btn-success btn-sm" disabled><i class="fas fa-check-circle"></i> Terima Kasih Terkirim</button>’; }
NOTIF.render();
}
function shareWA(scope) {
if (scope===‘day’&&S.date) { var res=getResDate(S.date).slice().sort(function(a,b){return(a.jam||’’).localeCompare(b.jam||’’);}); window.open(‘https://wa.me/?text=’+encodeURIComponent(buildDailyMsg(S.date,res)),’_blank’,‘noopener’); }
}
...
handle: null,
getPending: function() {
var now = Date.now(), seven = new Date(now - 7*24*3600*1000).toISOString().split(‘T’)[0], today = todayStr();
...
if (!p.length) { dot.style.display=‘none’; listEl.innerHTML=’<div class="nd-empty"><i class="fas fa-check-circle" style="color:#10b981;font-size:1.3rem;display:block;margin-bottom:6px"></i>Semua beres!</div>’; return; }
...
var opts = { menu:document.getElementById(‘po-menu’).checked, hp:document.getElementById(‘po-hp’).checked, dp:document.getElementById(‘po-dp’).checked, note:document.getElementById(‘po-note’).checked };
var items = res.map(function(r,i) {
var menuH = ‘’;
if (opts.menu&&Array.isArray(r.menus)&&r.menus.length) { menuH=’<p style="font-weight:600;margin-top:8px">🍽 Pesanan:</p><ul>’+r.menus.map(function(m){var md=getMenuByName(m.name),det=(md&&md.details)||[];return’<li><strong>’+m.quantity+‘x ‘+esc(m.name)+’</strong>’+(det.length?’<br><small style="color:#555">’+det.map(esc).join(’, ‘)+’</small>’:’’)+’</li>’;}).join(’’)+’</ul>’; }
return ‘<div class="card"><h3>’+(i+1)+’. ‘+esc(r.nama)+’</h3><p>⏰ ‘+r.jam+’  |  📍 ‘+esc(r.tempat)+’  |  👥 ‘+r.jumlah+’ orang</p>’+(opts.hp&&r.nomorHp?’<p>📱 ‘+r.nomorHp+’</p>’:’’)+menuH+(opts.dp&&parseInt(r.dp)>0?’<p>💰 DP: Rp’+formatRp(r.dp)+(r.tipeDp?’ (’+esc(r.tipeDp)+’)’:’’)+’</p>’:’’)+(opts.note&&r.tambahan?’<p>📝 ‘+esc(r.tambahan)+’</p>’:’’)+’</div>’;
}).join(’’);
...
var w = window.open(’’,’_blank’,‘noopener’); if (!w) { showToast(‘Pop-up diblokir!’,‘error’); return; }
...
var code = ‘’; try { code = btoa(unescape(encodeURIComponent(JSON.stringify(payload)))); } catch(e) {}
val(‘export-out’, code); val(‘import-in’, ‘’); openModal(‘modal-export’);
}
function copyExport() {
var el = document.getElementById(‘export-out’); if (!el||!el.value) { showToast(‘Tidak ada data’,‘error’); return; }
if (navigator.clipboard&&navigator.clipboard.writeText) navigator.clipboard.writeText(el.value).then(function(){showToast(‘Kode backup disalin! 📋’,‘success’);}).catch(function(){el.select();document.execCommand(‘copy’);showToast(‘Disalin!’,‘success’);});
else { el.select(); document.execCommand(‘copy’); showToast(‘Disalin!’,‘success’); }
}
async function doImport() {
var code = gval(‘import-in’).trim(); if (!code) { showToast(‘Tempel kode dulu!’,‘error’); return; }
var payload; try { payload = JSON.parse(decodeURIComponent(escape(atob(code)))); } catch(e) { showToast(’Kode tidak valid: ’+e.message,‘error’); return; }
...
S.biz = payload.biz||S.biz; S.menus = payload.menus||{}; S.locs = payload.locs||payload.locations||{}; S.res = payload.res||payload.reservations||{};
if (payload.ops)    S.ops    = Object.assign(S.ops, payload.ops);
if (payload.appear) S.appear = Object.assign(S.appear, payload.appear);
await saveBizFS(); DB.set(K.MENUS,S.menus); DB.set(K.LOCS,S.locs); DB.set(K.RES,S.res); await saveOpsFS(); DB.set(K.APPEAR,S.appear);
showToast(‘Data berhasil diimport! ✅’,‘success’); closeModal(‘modal-export’);
var biz = S.biz.name||‘Usaha Saya’; setText(‘cal-title’,’Dashboard - ’+biz); setText(‘sb-biz-name’,biz);
...
var y = parseInt(document.getElementById(‘anl-year’).value), mv = document.getElementById(‘anl-month’).value;
var filtered, mode;
...
var cnt=filtered.length, pax=filtered.reduce(function(s,r){return s+(parseInt(r.jumlah)||0);},0), dp=filtered.reduce(function(s,r){return s+(parseInt(r.dp)||0);},0);
...
var labels=[], data=[];
if (mode===‘month’) { var mc=Array(12).fill(0); filtered.forEach(function(r){if(r.date)mc[parseInt(r.date.split(’-’)[1])-1]++;}); labels=MONTHS_S; data=mc; }
else { var dm=new Date(y,parseInt(mv)+1,0).getDate(),dc=Array(dm).fill(0); filtered.forEach(function(r){if(r.date)dc[parseInt(r.date.split(’-’)[2])-1]++;}); labels=dc.map(function(_,i){return String(i+1);}); data=dc; }
...
var menuMap={}; filtered.forEach(function(r){if(Array.isArray(r.menus))r.menus.forEach(function(m){menuMap[m.name]=(menuMap[m.name]||0)+(parseInt(m.quantity)||1);});});
...
var custM={}; filtered.forEach(function(r){if(r.nomorHp){if(!custM[r.nomorHp])custM[r.nomorHp]={name:r.nama,count:0};custM[r.nomorHp].count++;}});
...
function anlCard(v,l,icon) { return ‘<div class="anl-card"><div style="font-size:.72rem;color:var(--ac);margin-bottom:7px"><i class="'+icon+'"></i></div><div class="anl-val">’+v+’</div><div class="anl-lbl">’+l+’</div></div>’; }
...
options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, title:{display:true,text:title,font:{size:13,weight:‘600’},color:‘rgba(255,255,255,.4)’,padding:{bottom:14}}, tooltip:{backgroundColor:’#18181b’,padding:10,cornerRadius:8,callbacks:{label:function(c){return’ ‘+c.raw+’ reservasi’;}}} }, scales:{ x:{grid:{display:false},ticks:{font:{size:10}}}, y:{beginAtZero:true,ticks:{stepSize:1,font:{size:10}},grid:{color:‘rgba(128,128,128,.08)’}} } }
});
}
...
═══════════════════════════════════════════════════ */
function loadBcView() {
var saved = localStorage.getItem(_ckey(K.BC_MSG)) || ‘’; val(‘bc-msg’, saved);
var map = {}; getAllRes().forEach(function(r){if(r.nomorHp&&!map[r.nomorHp])map[r.nomorHp]={name:r.nama,phone:r.nomorHp};});
S.bcList = Object.values(map).sort(function(a,b){return a.name.localeCompare(b.name);}); renderBcList(S.bcList);
}
function saveBcMsg() {
var msg = (gval(‘bc-msg’)||’’).trim(); if (!msg){showToast(‘Pesan kosong!’,‘error’);return;}
localStorage.setItem(_ckey(K.BC_MSG), msg); showToast(‘Pesan broadcast disimpan!’,‘success’);
}
...
el.innerHTML = list.map(function(c){return’<div class="bc-item"><div><div class="bc-name">’+esc(c.name)+’</div><div class="bc-phone">’+esc(c.phone)+’</div></div><button class="btn btn-wa btn-sm" data-phone="'+esc(c.phone)+'" data-name="'+esc(c.name)+'" onclick="sendBc(this)"><i class="fab fa-whatsapp"></i> Kirim</button></div>’;}).join(’’);
}
function filterBc(q) { var fl=q.toLowerCase(); renderBcList(!q?S.bcList:S.bcList.filter(function(c){return c.name.toLowerCase().includes(fl)||c.phone.includes(q);})); }
function sendBc(btn) {
var phone=btn.getAttribute(‘data-phone’),name=btn.getAttribute(‘data-name’);
var tpl=localStorage.getItem(_ckey(K.BC_MSG))||’’; if(!tpl){showToast(‘Atur pesan dulu!’,‘error’);return;}
openWA(phone, tpl.replace(/\bkak\b/gi,‘Kak *’+name+’*’));
btn.innerHTML=’<i class="fas fa-check"></i> Terkirim’; btn.className=‘btn btn-success btn-sm’; btn.disabled=true;
}
...
var sb=document.getElementById(‘sidebar’),ov=document.getElementById(‘sidebar-overlay’),open=sb&&sb.classList.contains(‘open’);
...
c.appendChild(div);
setTimeout(function() { div.style.opacity=‘0’; div.style.transform=‘translateX(18px)’; setTimeout(function(){if(div.parentNode)div.remove();},320); }, dur);
}
...
function nameColor(n)  { if(!n)return’#64748b’; var h=0; for(var i=0;i<n.length;i++)h=n.charCodeAt(i)+((h<<5)-h); return[’#e8630a’,’#06b6d4’,’#10b981’,’#f59e0b’,’#8b5cf6’,’#ec4899’,’#0284c7’,’#ef4444’][Math.abs(h)%8]; }
function validPhone(p) { return /^[\d\s-+()]{10,15}$/.test(p); }
function normPhone(p)  { if(!p)return’’; var c=p.replace(/\D/g,’’); return c.startsWith(‘0’)?‘62’+c.slice(1):c; }
