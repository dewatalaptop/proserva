...
/* ============================================================
2. APP INIT (after wizard or on return visit)
============================================================ */
function initApp () {
loadState();

// Header
var bizName = state.biz.name || ‘Usaha Saya’;
var el = document.getElementById(‘cal-biz-name’);
if (el) el.textContent = bizName;
var sub = document.getElementById(‘cal-subtitle’);
if (sub) sub.textContent = ‘Selamat datang kembali! Kelola reservasi ’ + bizName + ’ dengan mudah.’;
...
// Notif dropdown close on outside click
...
if (name === ‘analysis’)  { setupAnalysisSelectors(); runAnalysis(); }
if (name === ‘broadcast’) loadBroadcastView();

// Show/hide the topbar “Reservasi Baru” button
...
if (!name) { showToast(‘Nama usaha wajib diisi!’, ‘error’); return; }
...
if (wizardData.locations.some(function (l) { return l.name.toLowerCase() === name.toLowerCase(); })) {
showToast(‘Nama lokasi sudah ditambahkan!’, ‘error’);
return;
}
wizardData.locations.push({ name: name, capacity: cap });
...
renderWizardList(‘wz-locations-list’, wizardData.locations, function (i) {
...
if (!name) { showToast(‘Isi nama menu!’, ‘error’); return; }
if (wizardData.menus.some(function (m) { return m.name.toLowerCase() === name.toLowerCase(); })) {
showToast(‘Nama menu sudah ditambahkan!’, ‘error’);
return;
}
wizardData.menus.push({ name: name, price: price, details: details });
...
renderWizardList(‘wz-menus-list’, wizardData.menus, function (i) {
...
// Persist biz info
state.biz = { name: wizardData.bizName, type: wizardData.bizType };
saveBiz();

// Persist locations
wizardData.locations.forEach(function (loc) {
state.locations[genId()] = { name: loc.name, capacity: loc.capacity };
});
saveLocations();

// Persist menus
wizardData.menus.forEach(function (m) {
state.menus[genId()] = { name: m.name, price: m.price, details: m.details };
});
saveMenus();

// Mark setup done & init trial
...
document.getElementById(‘cal-month-label’).textContent = MONTHS_ID[m] + ’ ’ + y;
...
var d = parseInt(r.date.split(’-’)[2], 10);
dayCounts[d] = (dayCounts[d] || 0) + 1;
if (!dayNames[d]) dayNames[d] = [];
if (dayNames[d].length < 3) dayNames[d].push(r.nama || ‘?’);
});
...
var top = Object.entries(dayCounts).sort(function (a, b) { return b[1] - a[1]; })[0];
...
var pillHtml = count > 0
  ? '<div class="cal-res-pill"><i class="fas fa-calendar-check"></i> ' + count + '</div>'
  : '';

var namesHtml = names.length > 0
...
// Hide “Reservasi Baru” button in topbar while in detail view
var addBtn = document.getElementById(‘btn-add-res’);
...
// Menu section
var menuHtml = ‘<div style="color:var(--ink-4);font-size:0.82rem;font-style:italic;">Tidak ada pesanan</div>’;
...
// Info chips
var chips = ‘’;
if (r.nomorHp)  chips += ‘<div class="rc-info-chip"><i class="fas fa-phone"></i>’ + escapeHtml(r.nomorHp) + ‘</div>’;
if (r.dp > 0)   chips += ‘<div class="rc-info-chip"><i class="fas fa-money-bill-wave"></i>DP Rp’ + formatRupiah(r.dp) + (r.tipeDp ? ’ · ’ + escapeHtml(r.tipeDp) : ‘’) + ‘</div>’;
if (r.tambahan) chips += ‘<div class="rc-info-chip"><i class="fas fa-comment-dots"></i>’ + escapeHtml(r.tambahan) + ‘</div>’;

// Thank-you button
var thankBtn = ‘’;
if (r.nomorHp) {
if (r.thankYouSent) {
thankBtn = ‘<button class="btn-success-soft" disabled><i class="fas fa-check-circle"></i> Terima Kasih Terkirim</button>’;
} else {
thankBtn = ‘<button class="btn-secondary" style="font-size:0.78rem;padding:7px 11px;" onclick="sendThankYouById(\'' + r.id + '\')">’ +
‘<i class="fas fa-gift"></i> Ucapan Terima Kasih</button>’;
}
}
...
```
  ...
  '<div class="rc-top">' +
    '<div class="rc-name">' +
      '<div class="rc-avatar" style="background:' + avatarBg + ';">' + initials + '</div>' +
...
var result = !query ? all : all.filter(function (r) {
return (
...
mc.innerHTML = ‘’;
if (Array.isArray(r.menus) && r.menus.length > 0) {
r.menus.forEach(function (item) { addMenuRow(‘res-menus-container’, item.name, item.quantity); });
} else {
addMenuRow(‘res-menus-container’);
}
...
showFieldError(‘err-jumlah’, ‘Melebihi kapasitas lokasi (maks. ’ + loc.capacity + ’ orang)’);
valid = false;
}
}
...
if (!sel || !sel.value) return; // skip blank rows silently
if (!qty || qty < 1) {
showFieldError(‘err-menus’, ‘Jumlah menu minimal 1’);
menuValid = false; return;
}
if (usedNames[sel.value]) {
showFieldError(‘err-menus’, ‘Menu ’ + sel.value + ’ dipilih lebih dari satu kali’);
menuValid = false; return;
}
usedNames[sel.value] = true;
...
if (!existing) { showToast(‘Reservasi tidak ditemukan!’, ‘error’); return; }
var updated = Object.assign({}, existing, {
nama: nama, nomorHp: normalizePhone(hp),
jam: jam, jumlah: jumlah,
dp: dp, tipeDp: tipeDp,
tempat: tempat, tambahan: tambahan,
menus: menus
});
...
‘Belum ada menu. <a onclick="showView(\'menus\')" style="color:var(--accent);cursor:pointer;">Tambah menu dulu</a></div>’;
return;
}
...
return ‘<option value=”’ + escapeHtml(m.name) + ‘”’ + sel + ‘>’ + escapeHtml(m.name) + price + ‘</option>’;
}).join(’’);
...
‘<option value="">Pilih menu…</option>’ + opts +
‘</select>’ +
...
if (menuName) updateMenuRowPrice(div.querySelector(‘select’));
}
...
priceEl.textContent = (m && m.price) ? ‘Rp’ + formatRupiah(m.price) : ‘’;
}
...
if (hint) hint.textContent = loc ? ‘Kapasitas: ’ + loc.capacity + ’ orang’ : ‘’;
}
...
if (menus.length === 0) {
tbody.innerHTML =
‘<tr><td colspan="4" style="text-align:center;padding:36px;color:var(--ink-4);">’ +
...
‘</td>’ +
‘<td><span style="font-size:0.82rem;color:var(--ink-3);">’ +
(m.details && m.details.length ? m.details.map(escapeHtml).join(’, ‘) : ‘—’) +
‘</span></td>’ +
‘<td>’ +
...
return entry[1].name.toLowerCase() === name.toLowerCase() && entry[0] !== editId;
});
if (isDupe) { showToast(‘Nama menu sudah ada!’, ‘error’); return; }

if (editId) {
state.menus[editId] = { name: name, price: price, details: details };
} else {
state.menus[genId()] = { name: name, price: price, details: details };
}
...
if (!confirm(‘Hapus menu “’ + name + ‘”?\n\nMenu ini juga akan hilang dari semua reservasi yang sudah ada.’)) return;
...
if (locs.length === 0) {
tbody.innerHTML =
‘<tr><td colspan="3" style="text-align:center;padding:36px;color:var(--ink-4);">’ +
...
‘<td><span class="chip chip-blue"><i class="fas fa-users"></i> ’ + loc.capacity + ’ orang</span></td>’ +
‘<td>’ +
‘<div style="display:flex;gap:6px;">’ +
...
if (!capacity || capacity < 1) { showToast(‘Kapasitas minimal 1 orang!’, ‘error’); return; }

var isDupe = Object.entries(state.locations).some(function (entry) {
return entry[1].name.toLowerCase() === name.toLowerCase() && entry[0] !== editId;
});
if (isDupe) { showToast(‘Nama lokasi sudah ada!’, ‘error’); return; }

if (editId) {
state.locations[editId] = { name: name, capacity: capacity };
} else {
state.locations[genId()] = { name: name, capacity: capacity };
}
...
var name = state.locations[id] ? state.locations[id].name : ‘lokasi ini’;
...
if (list.length === 0) {
tbody.innerHTML =
‘<tr><td colspan="5" style="text-align:center;padding:36px;color:var(--ink-4);">’ +
(filter ? ‘Tidak ada hasil untuk “’ + escapeHtml(filter) + ‘”’ : ‘Belum ada data pelanggan.’) +
‘</td></tr>’;
return;
}
...
var r = findReservationById(id);
if (!r || !r.nomorHp) { showToast(‘Nomor HP tidak tersedia’, ‘error’); return; }
...
// Mark as sent
var updated = Object.assign({}, r, { thankYouSent: true });
updateReservation(updated);
...
‘<i class="fas fa-check-circle"></i> Terima Kasih Terkirim</button>’;
}
}
...
.slice().sort(function (a, b) { return (a.jam || ‘’).localeCompare(b.jam || ‘’); });
...
13. PRINT
============================================================ */
function showPrintOptions () { openModal(‘modal-print’); }
...
if (!el || !el.value) { showToast(‘Tidak ada data untuk disalin’, ‘error’); return; }
...
if (monthVal === ‘all’) {
filtered   = allRes.filter(function (r) { return r.date && r.date.startsWith(String(year)); });
chartMode  = ‘month’;
chartMonth = null;
} else {
...
chartData.labels,
chartData.data,
chartMode === ‘month’ ? ‘Reservasi per Bulan’ : ‘Reservasi per Tanggal’
);
...
‘<ul>’ + insights.map(function (i) { return ‘<li>’ + i + ‘</li>’; }).join(’’) + ‘</ul>’;

// Frequent customers
var custCounts = {};
filtered.forEach(function (r) {
if (!r.nomorHp) return;
if (!custCounts[r.nomorHp]) custCounts[r.nomorHp] = { name: r.nama, count: 0 };
custCounts[r.nomorHp].count++;
});
var topCusts = Object.values(custCounts).sort(function (a, b) { return b.count - a.count; }).slice(0, 5);
...
return (
‘<div class="anl-card">’ +
‘<div style="font-size:0.75rem;color:var(--accent);margin-bottom:8px;"><i class="' + icon + '"></i></div>’ +
...
if (_anlChartInstance) { _anlChartInstance.destroy(); _anlChartInstance = null; }
...
if (!area) return ‘rgba(232,99,10,0.7)’;
...
title:  { display: true, text: title, font: { size: 13, weight: ‘600’ }, color: ‘#3f3f46’, padding: { bottom: 16 } },
tooltip: {
...
cornerRadius:    8,
callbacks: {
label: function (ctx) { return ’ ’ + ctx.raw + ’ reservasi’; }
}
}
},
scales: {
...
y: {
beginAtZero: true,
ticks: { stepSize: 1, color: ‘#a1a1aa’, font: { size: 11 } },
...
map[r.nomorHp] = { name: r.nama, phone: r.nomorHp };
}
});
state.bcList = Object.values(map).sort(function (a, b) {
...
‘Belum ada data pelanggan dengan nomor HP.</div>’;
return;
}
...
if (!template) { showToast(‘Atur pesan broadcast dulu!’, ‘error’); return; }
