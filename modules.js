...
============================================================ */
var TRIAL = {
DURATION_MS: 7 * 24 * 60 * 60 * 1000,   // 7 days in ms
DURATION_DAYS: 7,
...
- If expired: wipe all data, reset setup flag, reload.
  */
  ...
  if (!DB.get(KEYS.SETUP_DONE)) return;   // not set up yet
  if (!this.isExpired()) return;           // still valid

```
// Wipe everything
DB.clear();
// Show a friendly alert before reload
alert(
...
var idx = state.reservations[mk].findIndex(function (r) { return r.id === res.id; });
...
var idx = state.reservations[mk].findIndex(function (r) { return r.id === id; });
if (idx !== -1) {
state.reservations[mk].splice(idx, 1);
// Clean up empty month buckets
if (state.reservations[mk].length === 0) delete state.reservations[mk];
saveReservations();
return true;
}
}
...
for (var mk in state.reservations) {
var found = state.reservations[mk].find(function (r) { return r.id === id; });
...
var key = r.nomorHp ? r.nomorHp : (’**noPhone**’ + (r.nama || ‘’).toLowerCase().trim());
...
map[key].count++;
map[key].totalPax += parseInt(r.jumlah) || 0;
...
return Object.values(state.menus).find(function (m) { return m.name === name; }) || null;
}

/** Find a location object by name */
function getLocationByName (name) {
return Object.values(state.locations).find(function (l) { return l.name === name; }) || null;
}
...
var MONTHS_SHORT = MONTHS_ID.map(function (m) { return m.slice(0, 3); });

var DAYS_ID = [‘Minggu’,‘Senin’,‘Selasa’,‘Rabu’,‘Kamis’,‘Jumat’,‘Sabtu’];

/**

- Format a date string (YYYY-MM-DD) to “15 Januari 2025”
  */
...
- Format “YYYY-MM-DD” → “Senin, 15 Jan 2025”
  */
  ...
  var p   = dateStr.split(’-’);
  return dow + ’, ’ + parseInt(p[2], 10) + ’ ’ + MONTHS_SHORT[parseInt(p[1], 10) - 1] + ’ ’ + p[0];
  }

/**

- Format number as Indonesian Rupiah with dots: 1.500.000
  */
...
- Compact Rupiah for stat cards: 1.5jt, 250rb
  */
...
/** Close any open modal when clicking outside its inner panel */
...
- phone: raw digits string (081… or 628…)
- msg:   plain-text message (will be URI-encoded)
  */
...
- Build a reservation confirmation message
  */
...
‘🍽 *Pesanan:*\n’ + menuList + ‘\n\n’ +
(r.dp > 0 ? ‘💰 *DP:* Rp’ + formatRupiah(r.dp) + (r.tipeDp ? ’ via ’ + r.tipeDp : ’ ✅’) + ‘\n\n’ : ‘’) +
...
- Build a daily summary message for WA share
  */
...
- Encode current data to a base64 backup string
  */
  function exportData () {
  var payload = {
  v:           2,
...
- Decode and apply a backup string.
- Returns { ok: true } or { ok: false, error: string }
  */
  function importData (code) {
  try {
  var payload = JSON.parse(decodeURIComponent(escape(atob(code.trim()))));
  if (!payload.v) return { ok: false, error: ‘Format backup tidak valid (missing version).’ };
  ...
  return { ok: false, error: ’Gagal memproses kode backup: ’ + e.message };
  }
  }
...
/** Build list of reservations needing a thank-you */
getPendingThankYous: function () {
var now         = Date.now();
var sevenAgo    = new Date(now - 7 * 24 * 3600 * 1000).toISOString().split(‘T’)[0];
var today       = todayStr();
...
return getAllReservations().filter(function (r) {
  ...
  // Only surface if the reservation time has passed by ≥3 h
...
/** Refresh the bell badge and dropdown list */
render: function () {
...
if (pending.length === 0) {
  ...
  listEl.innerHTML  =
    '<div class="nd-empty">' +
    '<i class="fas fa-check-circle"></i><br/>Semua ucapan terima kasih sudah terkirim!</div>';
  return;
}
...
listEl.innerHTML  = pending.map(function (r) {
  return (
    '<div class="notif-item" id="ni-' + r.id + '">' +
      ...
      '<div class="ni-date">' + formatDateDisplay(r.date) + ' · ' + r.jam + '</div>' +
...
self.intervalHandle = setInterval(function () { self.render(); }, 2 * 60 * 1000);
}
};

/* ============================================================
17. PRINT HELPER
Generates a standalone HTML document and opens print dialog
============================================================ */
...
var md      = getMenuByName(item.name);
var details = md ? md.details : [];
return ‘<li><strong>’ + item.quantity + ‘x ’ + escapeHtml(item.name) + ‘</strong>’ +
(details.length
? ‘<ul style="color:#555;">’ + details.map(function (d) { return ‘<li>’ + escapeHtml(d) + ‘</li>’; }).join(’’) + ‘</ul>’
: ‘’) +
‘</li>’;
}).join(’’) + ‘</ul>’;
}
...
return (
  '<div class="card">' +
    ...
    '<p>⏰ ' + r.jam + ' &nbsp;|&nbsp; 📍 ' + escapeHtml(r.tempat) +
      ' &nbsp;|&nbsp; 👥 ' + r.jumlah + ' orang</p>' +
    (opts.hp  && r.nomorHp  ? '<p>📱 ' + r.nomorHp  + '</p>' : '') +
    menuHtml +
    (opts.dp  && r.dp > 0   ? '<p>💰 DP: Rp' + formatRupiah(r.dp) + (r.tipeDp ? ' (' + escapeHtml(r.tipeDp) + ')' : '') + '</p>' : '') +
    (opts.tambahan && r.tambahan ? '<p>📝 ' + escapeHtml(r.tambahan) + '</p>' : '') +
  '</div>'
);
```
...
‘h1{font-size:1.4rem;margin-bottom:2px;} .meta{color:#71717a;font-size:0.875rem;margin-bottom:24px;}’ +
‘.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:16px;}’ +
‘.card{border:1px solid #e4e4e7;border-radius:12px;padding:16px;break-inside:avoid;}’ +
‘h3{margin:0 0 8px;font-size:1rem;color:#18181b;}’ +
‘p{margin:4px 0;font-size:0.85rem;} ul{margin:4px 0;padding-left:18px;font-size:0.82rem;}’ +
...
‘<p class="meta">’ + formatDateFull(dateStr) + ’  ·  ’ + reservations.length + ’ reservasi</p>’ +
...
/** Simple HTML escape to prevent XSS in dynamic innerHTML */
...
/** Convert a name to a consistent avatar color (hsl) */
...
// ESC → close top-most modal / notif dropdown
if (e.key === ‘Escape’) {
...
/* ============================================================
21. SETUP WIZARD DATA STORE
Temporary holder during the 3-step wizard
============================================================ */

var wizardData = {
bizName:   ‘’,
bizType:   ‘restoran’,
locations: [],   // [{name, capacity}]
menus:     []    // [{name, price, details:[]}]
};
...
- Compute summary stats for an array of reservations
- Returns { count, totalPax, totalDp, avgPax }
  */
...
- Returns sorted [{key, count}] descending
  */
...
- Count menu quantities across reservations
- Returns sorted [{key (name), count (qty)}] descending
  */
  ...
  map[m.name] = (map[m.name] || 0) + (parseInt(m.quantity) || 1);
  });
  });
  return Object.entries(map)
  .map(function (e) { return { key: e[0], count: e[1] }; })
...
- mode: ‘month’ → 12 buckets (Jan–Dec); ‘day’ → days in month
  */
  function buildChartData (reservations, mode, year, month) {
  if (mode === ‘month’) {
  ...
  var byDay       = new Array(daysInMonth).fill(0);
  reservations.forEach(function (r) {
  if (!r.date) return;
  var d = parseInt(r.date.split(’-’)[2], 10) - 1;
  if (d >= 0 && d < daysInMonth) byDay[d]++;
  });
  return {
  labels: Array.from({ length: daysInMonth }, function (_, i) { return i + 1; }),
  data:   byDay
  };
  }
  }
...
- Generate AI-style insights from stats
  */
  function generateInsights (reservations, stats) {
  var insights = [];

if (reservations.length === 0) {
return [‘Belum ada data pada periode ini. Tambahkan reservasi untuk melihat insight!’];
}

// Busiest day of week
var byDow = countBy(reservations, function (r) {
return r.date ? DAYS_ID[new Date(r.date + ‘T12:00:00’).getDay()] : null;
});
if (byDow.length > 0) {
insights.push(
‘Hari tersibuk adalah <strong>’ + byDow[0].key + ‘</strong> ’ +
‘(’ + byDow[0].count + ’ reservasi). Siapkan staf ekstra pada hari tersebut.’
);
}
...
insights.push(
‘Menu paling diminati: <strong>’ + topMenus[0].key + ‘</strong> ’ +
‘(’ + topMenus[0].count + ’ porsi). Pastikan stok bahan selalu tersedia.’
);
}

// Avg pax
var avg = parseFloat(stats.avgPax);
if (avg >= 7) {
insights.push(
‘Rata-rata grup cukup besar (’ + avg + ’ orang). ’ +
‘Pertimbangkan paket grup atau keluarga dengan harga spesial.’
);
} else if (avg > 0 && avg < 4) {
insights.push(
’Banyak kunjungan pasangan atau grup kecil (rata-rata ’ + avg + ’ orang). ’ +
‘Paket couple atau sahabat mungkin sangat menarik.’
);
}

// DP conversion
var withDp = reservations.filter(function (r) { return parseInt(r.dp) > 0; }).length;
if (reservations.length > 0) {
var dpPct = Math.round((withDp / reservations.length) * 100);
if (dpPct < 50) {
insights.push(
’Hanya ’ + dpPct + ’% reservasi membayar DP. ’ +
‘Dorong lebih banyak pelanggan untuk DP agar no-show berkurang.’
);
} else {
insights.push(
dpPct + ’% reservasi sudah membayar DP — ’ +
‘tingkat komitmen pelanggan sangat baik! 🎉’
);
}
}
...
- Personalize broadcast message by replacing “kak” with “Kak *Name*”
  */
...
/* ============================================================
EXPORTS (globals for app.js)
============================================================ */
// All functions are on the global scope (no module system needed)
// app.js can call any of them directly.